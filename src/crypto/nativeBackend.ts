 ...
 * X25519 and XEdDSA paths. Selected via `HOSHINO_NATIVE_BACKEND`:
 *
 *   - `auto` (default) – try the NAPI addon first, then the WASM build,
 *                        then no accelerator.
 *   - `napi`           – only the compiled Rust NAPI addon (`@revizahoshii/hoshino-native`).
 ...
 * warning with code `HOSHINO_NATIVE_LOAD_FAILED` so the degradation is
 * visible. The per-primitive `HOSHINO_X25519_FORCE_JS` / `HOSHINO_XEDDSA_FORCE_JS`
...
const WASM_MODULE_SPECIFIER = '@revizahoshii/hoshino-native/wasm/pkg/hoshino_native_wasm.js'
const WASM_BINARY_SPECIFIER = '@revizahoshii/hoshino-native/wasm/pkg/hoshino_native_wasm_bg.wasm'
...
const ABSENCE_TOKENS = {
    napi: ["'@revizahoshii/hoshino-native'", 'binding.js', 'Cannot find native binding'],
    wasm: ["'@revizahoshii/hoshino-native'", 'hoshino_native_wasm.js']
} as const
...
function warnLoadFailure(backend: string, error: unknown): void {
    process.emitWarning('failed to load native crypto accelerator, falling back', {
        code: 'HOSHINO_NATIVE_LOAD_FAILED',
...
export function resolveNativeCryptoBackend(): NativeCryptoModule | null {
    ...
    const backend = (process.env.HOSHINO_NATIVE_BACKEND ?? 'auto').toLowerCase()
    cached = null
    if (backend === 'js' || backend === 'none') return cached
    if (backend === 'napi' || backend === 'auto') {
        try {
            cached = require('@revizahoshii/hoshino-native') as NativeCryptoModule
        } catch (error) {
