...
class WaClientImpl extends EventEmitter {
    ...
    public constructor(options: WaClientOptions, logger: Logger = new ConsoleLogger('info')) {
        ...
        this.on('connection', (event) => {
            ...
            this.logger.warn(
                'wa rejected the connect with client_too_old: the hoshino default WA version is outdated. ' +
                    'Auto-recovering by fetching the current version from the public source – ' +
                    'please upgrade hoshino so the shipped default catches up.'
            )
 ...
 * @example
 * ```ts
 * import { createPinoLogger, createStore, WaClient } from '@revizahoshii/hoshino'
