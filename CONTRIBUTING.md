...
[`README.md`](README.md) and at [hoshino.to](https://hoshino.to/) (source:
[`revizahoshii-no/lreviza-baileys-docs`](https://github.com/revizahoshii-no/lreviza-baileys-docs)).
...
`hoshino` is an independent runtime implementation (not a wrapper/fork of an
...
```bash
git clone https://github.com/revizahoshii-no/lreviza-baileys
cd hoshino
npm install
npm run example
```
...
- Typed store contracts in `src/store/contracts/`; persistent + cache
  providers live under `@revizahoshii/hoshino-store-*` packages
...
- `typecheck-packages` across every `@revizahoshii/hoshino-*` workspace
...
End-user guides and the API reference at [hoshino.to](https://hoshino.to/) are
built from [`revizahoshii-no/lreviza-baileys-docs`](https://github.com/revizahoshii-no/lreviza-baileys-docs).
