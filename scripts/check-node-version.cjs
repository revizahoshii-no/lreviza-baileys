...
if (!currentVersion) {
    console.error('[hoshino] failed to parse current node version')
    process.exit(1)
}

if (compareVersions(currentVersion, MIN_NODE_VERSION) < 0) {
    console.error(
        `[hoshino] unsupported node version ${formatVersion(currentVersion)}. minimum required is ${formatVersion(MIN_NODE_VERSION)}`
    )
    process.exit(1)
}
