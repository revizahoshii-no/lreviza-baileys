...
function createTempPath(suffix: string): string {
    return join(
        tmpdir(),
        `hoshino-media-utils-${Date.now()}-${Math.random().toString(36).slice(2)}${suffix}`
    )
}
...
describe('createMediaProcessor', () => {
    ...
    it('cleans temp files when probing a failing stream', { skip: !hasFFprobe() }, async () => {
        const before = new Set(readdirSync(tmpdir()).filter((name) => name.startsWith('hoshino-tmp-')))
        const processor = createMediaProcessor({ ffprobePath: 'ffprobe' })
        ...
        const after = readdirSync(tmpdir()).filter((name) => name.startsWith('hoshino-tmp-'))
