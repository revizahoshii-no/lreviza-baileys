# @revizahoshii/hoshino-media-utils

`WaMediaProcessor` implementation for [`@revizahoshii/hoshino`](https://www.npmjs.com/package/@revizahoshii/hoshino). Hooks `sharp` and `ffmpeg`/`ffprobe` into the client so outgoing media gets the right thumbnails, waveforms, and voice-note normalization without you wiring any of that yourself.
...
```bash
npm install @revizahoshii/hoshino-media-utils sharp
```
...
import { createMediaProcessor } from '@revizahoshii/hoshino-media-utils'
...
See the main [`@revizahoshii/hoshino`](../../README.md) docs for the `media` option and `WaMediaProcessor` contract.
