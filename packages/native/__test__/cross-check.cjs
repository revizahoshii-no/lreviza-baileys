...
// HOSHINO_NATIVE_BACKEND selects the backend under test (auto / napi / wasm).
// Run from repo root via tsx so the @crypto path aliases resolve:
//   node --import tsx packages/native/__test__/cross-check.cjs

// Force the JS implementations on the @revizahoshii/hoshino side BEFORE it loads, so the
// comparison is accelerator vs pure JS instead of accelerator vs itself.
process.env.HOSHINO_XEDDSA_FORCE_JS = '1'
process.env.HOSHINO_X25519_FORCE_JS = '1'
...
async function main() {
    ...
    if (
        ...
        const backend = process.env.HOSHINO_NATIVE_BACKEND ?? 'auto'
        console.error(`no accelerator backend loaded (HOSHINO_NATIVE_BACKEND=${backend})`)
        process.exit(2)
    }
