# @revizahoshii/hoshino-native

Optional native accelerators for [`@revizahoshii/hoshino`](https://www.npmjs.com/package/@revizahoshii/hoshino). Moves the crypto hot path off pure JavaScript and onto a compiled backend, with transparent fallback so the client works everywhere regardless of what's installed.
...
| **js**   | Pure-JS / `node:crypto` (no accelerator) | Nothing – built into `@revizahoshii/hoshino`        |

If the native addon can't load, the WASM fallback is used; if that's disabled,
the pure-JS path in `@revizahoshii/hoshino` takes over. Installing this package is always
safe: a missing or unsupported binary never breaks the client.

## Install

```bash
npm install @revizahoshii/hoshino-native
```
...
the pure-JS path. To pin a single backend, set `HOSHINO_NATIVE_BACKEND` before
the process starts:

```bash
HOSHINO_NATIVE_BACKEND=napi   # only the native addon
HOSHINO_NATIVE_BACKEND=wasm   # only the WebAssembly build
HOSHINO_NATIVE_BACKEND=js     # disable the accelerator, use the pure-JS path
```
...
- **Node**: `@revizahoshii/hoshino` loads it automatically (the glue is ESM, loaded via
...
import init, {
    ...
    xeddsaSign,
    xeddsaVerify
} from '@revizahoshii/hoshino-native/wasm/pkg/hoshino_native_wasm.js'
...
- **The root import is the NAPI addon.** `require('@revizahoshii/hoshino-native')` resolves
  to the compiled addon, so on the published wasm-only package it throws a
  module-not-found error – that failure is exactly what makes `@revizahoshii/hoshino` fall
  through to the WASM build. To use the primitives directly, import the
  `wasm/pkg` subpath shown above.

See the main [`@revizahoshii/hoshino`](../../README.md) docs for the client contract.
