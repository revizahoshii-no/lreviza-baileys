...
export interface RunMcpServerOptions {
    /** Server name shown in the MCP client UI. Defaults to `'@revizahoshii/hoshino-mcp-server'`. */
...
const buildMcpServer = (
    ...
    const server = new sdk.Server(
        {
            name: options.name ?? '@revizahoshii/hoshino-mcp-server',
...
const runHttpTransport = async (
    ...
    if (!LOOPBACK_HOSTS.has(config.httpHost)) {
        ...
        // WaClient + @revizahoshii/hoshino surface. Binding to a non-loopback interface puts
 ...
 *
 * Used by the published `hoshino-mcp-server` binary; you only need to call
