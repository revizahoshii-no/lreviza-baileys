# @revizahoshii/hoshino-store-mongo

MongoDB-backed persistent store for [`@revizahoshii/hoshino`](https://www.npmjs.com/package/@revizahoshii/hoshino). Good fit for fleets that already operate on MongoDB and want session state to ride that same infrastructure (replica sets, Atlas, etc.).
...
```bash
npm install @revizahoshii/hoshino-store-mongo mongodb
```
...
import { createMongoStore } from '@revizahoshii/hoshino-store-mongo'
...
- The TTL-monitor latency means cache entries can outlive the TTL by up to ~1 minute. Acceptable for `groupMetadata`/`deviceList`; for tighter eviction prefer `@revizahoshii/hoshino-store-redis`.
- Indexes are created lazily on first touch per collection (one-time cost). Subsequent process restarts skip the create step.

See the main [`@revizahoshii/hoshino`](../../README.md) docs for the full store contract.
