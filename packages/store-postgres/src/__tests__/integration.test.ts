...
import type { SignalAddress, SignalSessionRecord } from '@revizahoshii/hoshino/signal'
...
const host = process.env.HOSHINO_TEST_PG_HOST
const port = process.env.HOSHINO_TEST_PG_PORT
let sessionSeed = 0
...
describe('store-postgres integration', { timeout: 60_000 }, () => {
    ...
    before(async () => {
        ...
        pool = createPgPool({
            host,
            port: Number(port),
            user: process.env.HOSHINO_TEST_PG_USER ?? 'postgres',
            password: process.env.HOSHINO_TEST_PG_PASSWORD ?? 'test',
            database: process.env.HOSHINO_TEST_PG_DATABASE ?? 'hoshino_test'
        })
    ...
    it('auth: save, load, clear', async (t) => {
        if (!store) return t.skip('HOSHINO_TEST_PG_* not set')
    ...
    it('signal: setSessionsBatch with batchInsertChunkSize=2 splits into power-of-two chunks', async (t) => {
        if (!pool) return t.skip('HOSHINO_TEST_PG_* not set')
    ...
    it('rejects invalid batchInsertChunkSize', (t) => {
        if (!pool) {
            t.skip('HOSHINO_TEST_PG_* not set')
            return
        }
        const livePool = pool
        assert.throws(
