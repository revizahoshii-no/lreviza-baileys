...
test('@revizahoshii/hoshino client completes a full noise XX handshake against the fake server', async () => {
    const server = await FakeWaServer.start()
    const { client } = createHoshinoClient(server, { sessionId: 'fake-server-test' })
