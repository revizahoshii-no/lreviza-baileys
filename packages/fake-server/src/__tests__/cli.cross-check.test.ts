...
import { createNoopLogger, createStore, WaClient, type WaClientEventMap } from '@revizahoshii/hoshino'
import { hexToBytes } from '@revizahoshii/hoshino/util'
...
test('cli banner shows pair mode off by default', async () => {
    ...
    try {
        const stdout = await cli.waitForStdout(
            (value) => value.includes('@revizahoshii/hoshino-fake-server is up'),
            'startup banner'
        )
