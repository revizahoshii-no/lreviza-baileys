...
import { createHoshinoMobileClient } from './helpers/hoshino-mobile-client'
...
test('link-code pairing runs end to end between two real clients', async () => {
    ...
    const { client: primary } = await createHoshinoMobileClient(server, {
        sessionId: 'link-code-primary',
        phoneNumber: PHONE
    })
    const { client: companion } = createHoshinoClient(server, {
...
test('a second account logging into the same session gets no companion-host wiring', async () => {
    const server = await FakeWaServer.start({ tcp: true })
    const { client: owner } = await createHoshinoMobileClient(server, {
        sessionId: 'link-code-owner',
        phoneNumber: PHONE
    })
    const intruderPhone = '5511970004444'
    const { client: intruder } = await createHoshinoMobileClient(server, {
