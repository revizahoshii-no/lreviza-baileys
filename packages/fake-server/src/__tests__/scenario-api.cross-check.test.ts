...
test('scenario.afterAuth fires once the noise handshake completes', async () => {
    ...
    const { client } = createHoshinoClient(server, { sessionId: 'scenario-after-auth' })
    try {
...
test('capturedStanzaSnapshot starts empty for the pairing flow', async () => {
    const server = await FakeWaServer.start()
    const { client } = createHoshinoClient(server, { sessionId: 'snapshot-empty' })
    try {
        await client.connect()
...
test('expectIq rejects with a clear timeout when no IQ matches', async () => {
    const server = await FakeWaServer.start()
    const { client } = createHoshinoClient(server, { sessionId: 'expect-iq-timeout' })
    try {
        await client.connect()
