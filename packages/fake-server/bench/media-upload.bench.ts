 ...
 *
 * Tunables:
 *   HOSHINO_BENCH_MEDIA_UPLOADS  (default 20)
 *   HOSHINO_BENCH_MEDIA_SIZES    (default "10240,102400,1048576,4194304")
...
function parseSizes(): readonly number[] {
    const raw = readCsvEnv('HOSHINO_BENCH_MEDIA_SIZES', [
        String(10 * 1024),
        String(100 * 1024),
        String(1024 * 1024),
        String(4 * 1024 * 1024)
...
async function main(): Promise<void> {
    ...
    const uploads = readPositiveIntEnv('HOSHINO_BENCH_MEDIA_UPLOADS', 20)
    const backendName = process.env.HOSHINO_BENCH_STORE ?? 'memory'
    const separate = argSet.has('--separate-process')

    console.log('@revizahoshii/hoshino media-upload bench')
    console.log('──────────────────────────')
