/**
 * End-to-end retry bench driven through @revizahoshii/hoshino-fake-server.
 *
 ...
 *
 * Tunables (env vars):
 *   HOSHINO_BENCH_RETRY_MESSAGES         (default 100)
 *   HOSHINO_BENCH_RETRY_SCENARIOS        (csv; default = all)
...
function readScenarioFilter(): ReadonlySet<string> {
    const raw = process.env.HOSHINO_BENCH_RETRY_SCENARIOS
    ...
    for (const part of raw.split(',')) {
        ...
        if (!ALL_SCENARIOS.has(trimmed)) {
            throw new Error(
                `unknown HOSHINO_BENCH_RETRY_SCENARIOS entry "${trimmed}"; valid: ${[...ALL_SCENARIOS].join(',')}`
            )
        }
        out.add(trimmed)
    }
...
function readConfig(): BenchConfig {
    return {
        messages: readPositiveIntEnv('HOSHINO_BENCH_RETRY_MESSAGES', 100),
...
async function main(): Promise<void> {
    ...
    console.log('@revizahoshii/hoshino retry bench')
    console.log('───────────────────')
...
async function runInProcess(
    ...
    const fixture = await bringUpPairedClient(storeFixture, { sessionId: 'hoshino-bench-retry' })
...
async function runSeparateProcess(
    ...
    const storeFixture = await buildBenchStore()
    const fixture = await bringUpPairedClientViaRpc(storeFixture, { sessionId: 'hoshino-bench-retry' })
