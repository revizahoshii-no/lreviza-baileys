/**
 * End-to-end messaging-with-media bench driven through @revizahoshii/hoshino-fake-server.
 *
 ...
 *
 * Scenarios (selectable via HOSHINO_BENCH_MEDIA_SCENARIOS):
 ...
 *
 * Tunables (env vars):
 *   HOSHINO_BENCH_MEDIA_MESSAGES         (default 50)
 *   HOSHINO_BENCH_MEDIA_BYTES            (default 65_536)
 *   HOSHINO_BENCH_MEDIA_CHUNK_BYTES      (default 65_536, stream chunk size)
 *   HOSHINO_BENCH_MEDIA_GROUP_MEMBERS    (default 8)
 *   HOSHINO_BENCH_MEDIA_INPUT            (stream | buffer; default stream)
 *   HOSHINO_BENCH_MEDIA_SCENARIOS        (csv; default = all)
 *   HOSHINO_BENCH_STORE                  (memory | sqlite | mysql | ...)
 *   HOSHINO_BENCH_VERBOSE                (=1 to print lib warn/error)
 *   HOSHINO_BENCH_JSON                   (=1 to also print as JSON)
 *
...
import type { WaClient, WaClientEventMap } from '@revizahoshii/hoshino'
...
function readScenarioFilter(): ReadonlySet<string> {
    const raw = process.env.HOSHINO_BENCH_MEDIA_SCENARIOS
    ...
    for (const part of raw.split(',')) {
        ...
        if (!ALL_SCENARIOS.has(trimmed)) {
            throw new Error(
                `unknown HOSHINO_BENCH_MEDIA_SCENARIOS entry "${trimmed}"; valid: ${[...ALL_SCENARIOS].join(',')}`
            )
        }
        out.add(trimmed)
    }
...
function readInputMode(): MediaInputMode {
    const raw = process.env.HOSHINO_BENCH_MEDIA_INPUT
    if (!raw) return 'stream'
    if (raw === 'stream' || raw === 'buffer') return raw
    throw new Error(`invalid HOSHINO_BENCH_MEDIA_INPUT: ${raw} (expected 'stream' or 'buffer')`)
}

function readConfig(): BenchConfig {
    return {
        messages: readPositiveIntEnv('HOSHINO_BENCH_MEDIA_MESSAGES', 50),
        payloadBytes: readPositiveIntEnv('HOSHINO_BENCH_MEDIA_BYTES', 64 * 1024),
        chunkBytes: readPositiveIntEnv('HOSHINO_BENCH_MEDIA_CHUNK_BYTES', 64 * 1024),
        groupMembers: readPositiveIntEnv('HOSHINO_BENCH_MEDIA_GROUP_MEMBERS', 8),
...
async function main(): Promise<void> {
    ...
    const backendName = process.env.HOSHINO_BENCH_STORE ?? 'memory'
    const separate = argSet.has('--separate-process')

    console.log('@revizahoshii/hoshino messaging-with-media bench')
    console.log('──────────────────────────────────')
...
async function runInProcess(
    ...
    const fixture = await bringUpPairedClient(storeFixture, { sessionId: 'hoshino-bench-media' })
...
async function runSeparateProcess(
    ...
    const storeFixture = await buildBenchStore()
    const fixture = await bringUpPairedClientViaRpc(storeFixture, { sessionId: 'hoshino-bench-media' })
