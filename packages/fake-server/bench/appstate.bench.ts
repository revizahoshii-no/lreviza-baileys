 ...
 *
 * Tunables:
 *   HOSHINO_BENCH_APPSTATE_OUTGOING  (default 200)
 *   HOSHINO_BENCH_APPSTATE_INCOMING  (default 500)
 *
...
async function main(): Promise<void> {
    ...
    const outgoingOps = readPositiveIntEnv('HOSHINO_BENCH_APPSTATE_OUTGOING', 200)
    const incomingOps = readPositiveIntEnv('HOSHINO_BENCH_APPSTATE_INCOMING', 500)
    const backendName = process.env.HOSHINO_BENCH_STORE ?? 'memory'
    const separate = argSet.has('--separate-process')

    console.log('@revizahoshii/hoshino appstate bench')
    console.log('──────────────────────')
