 ...
 *
 * Tunables:
 *   HOSHINO_BENCH_RECONNECT_ITERATIONS  (default 20)
 *
...
import { WaClient, type WaClientEventMap, type WaStore } from '@revizahoshii/hoshino'
...
async function main(): Promise<void> {
    ...
    const iterations = readPositiveIntEnv('HOSHINO_BENCH_RECONNECT_ITERATIONS', 20)
    const backendName = process.env.HOSHINO_BENCH_STORE ?? 'memory'
    const separate = argSet.has('--separate-process')

    console.log('@revizahoshii/hoshino reconnect-resume bench')
    console.log('──────────────────────────────')
