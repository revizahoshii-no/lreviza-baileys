 ...
 *
 * Tunables:
 *   HOSHINO_BENCH_GROUP_OPS         (default 100)  ops per scenario
 *   HOSHINO_BENCH_GROUP_MEMBERS_PER_CREATE (default 5) jids per group on create
 *
...
async function main(): Promise<void> {
    ...
    const config: GroupBenchConfig = {
        ops: readPositiveIntEnv('HOSHINO_BENCH_GROUP_OPS', 100),
        membersPerCreate: readPositiveIntEnv('HOSHINO_BENCH_GROUP_MEMBERS_PER_CREATE', 5)
    }
    const backendName = process.env.HOSHINO_BENCH_STORE ?? 'memory'
    const separate = argSet.has('--separate-process')

    console.log('@revizahoshii/hoshino group-provision bench')
    console.log('─────────────────────────────')
