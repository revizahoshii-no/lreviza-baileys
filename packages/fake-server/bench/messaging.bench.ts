/**
 * End-to-end messaging profiling for @revizahoshii/hoshino, driven through the
 * @revizahoshii/hoshino-fake-server. Pairs a single real `WaClient` against an
 ...
 *   HOSHINO_BENCH_CONTACTS              (default 1000)
 *   HOSHINO_BENCH_CONTACT_DEVICES       (default 2)
 *   HOSHINO_BENCH_GROUPS                (default 4)
 *   HOSHINO_BENCH_GROUP_MEMBERS         (default 500)
 *   HOSHINO_BENCH_MESSAGES              (default 1000)
 *   HOSHINO_BENCH_SCENARIOS             (csv of: send_1to1, recv_1to1,
 *                                     send_group, recv_group; default = all)
 *   HOSHINO_BENCH_JSON                  (=1 to also print results as JSON)
 *
...
import { type Logger, WaClient, type WaClientEventMap } from '@revizahoshii/hoshino'
...
const benchLogger: Logger = {
    ...
    info: () => {},
    warn: (...args: unknown[]) => {
        if (process.env.HOSHINO_BENCH_VERBOSE) console.warn('[lib warn]', ...args)
    },
    error: (...args: unknown[]) => {
        if (process.env.HOSHINO_BENCH_VERBOSE) console.error('[lib error]', ...args)
    },
    child: () => benchLogger
}
 ...
 * fake-server / @revizahoshii/hoshino modules are imported in main(), so module-
...
function readScenarioFilter(): ReadonlySet<string> {
    const raw = process.env.HOSHINO_BENCH_SCENARIOS
    ...
    for (const part of raw.split(',')) {
        ...
        if (!ALL_SCENARIOS.has(trimmed)) {
            throw new Error(
                `unknown HOSHINO_BENCH_SCENARIOS entry "${trimmed}"; valid: ${[...ALL_SCENARIOS].join(',')}`
            )
        }
        out.add(trimmed)
    }
...
function readConfig(): BenchConfig {
    return {
        contacts: readPositiveIntEnv('HOSHINO_BENCH_CONTACTS', 1_000),
        contactDevices: readPositiveIntEnv('HOSHINO_BENCH_CONTACT_DEVICES', 2),
        groups: readPositiveIntEnv('HOSHINO_BENCH_GROUPS', 4),
        groupMembers: readPositiveIntEnv('HOSHINO_BENCH_GROUP_MEMBERS', 500),
        messages: readPositiveIntEnv('HOSHINO_BENCH_MESSAGES', 1_000),
    ...
    const client = new WaClient(
        {
            ...
            sessionId: 'hoshino-messaging-bench',
...
function printConfig(config: BenchConfig): void {
    console.log('@revizahoshii/hoshino messaging bench')
    console.log('────────────────────────')
    ...
    console.log(`  store             : ${process.env.HOSHINO_BENCH_STORE ?? 'memory'}`)
...
async function mainSeparateProcess(
    ...
    const client = new WaClient(
        {
            store: storeFixture.store,
            sessionId: 'hoshino-bench-separate',
...
async function main(): Promise<void> {
    ...
    if (process.env.HOSHINO_BENCH_JSON === '1') {
        console.log(
            JSON.stringify(
                results.map((r) => ({
                    name: r.name,
