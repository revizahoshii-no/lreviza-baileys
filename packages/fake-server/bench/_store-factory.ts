 ...
 * backend via `HOSHINO_BENCH_STORE` (memory | sqlite | postgres | mysql |
 * redis | mongo) and wires the matching `@revizahoshii/hoshino-store-*` package into
 * `createStore()` from `@revizahoshii/hoshino`.
 *
 * - `memory` (default): in-process bounded maps. No external dep.
 * - `sqlite`: file or `:memory:`. Driver via `HOSHINO_BENCH_SQLITE_PATH`
 ...
 *   `HOSHINO_TEST_PG_*` / `HOSHINO_TEST_MYSQL_*` env vars used by
 *   `scripts/test-stores.cjs` and the per-package integration tests.
 * - `redis`: needs a live redis. Reads `HOSHINO_TEST_REDIS_HOST/PORT`.
 * - `mongo`: needs a live mongo (replica set). Reads
 *   `HOSHINO_TEST_MONGO_HOST/PORT`.
 *
...
import { createStore, type WaCreateStoreOptions, type WaStore } from '@revizahoshii/hoshino'
...
export function resolveStoreBackend(): StoreBackendName {
    const raw = process.env.HOSHINO_BENCH_STORE ?? 'memory'
    if (!ALL_BACKENDS.has(raw as StoreBackendName)) {
        throw new Error(`unknown HOSHINO_BENCH_STORE=${raw}; valid: ${[...ALL_BACKENDS].join(', ')}`)
    }
...
async function buildSqliteStore(): Promise<BenchStoreFixture> {
    const { createSqliteStore } = await import('@revizahoshii/hoshino-store-sqlite')
    const userPath = process.env.HOSHINO_BENCH_SQLITE_PATH
    const ownsFile = !userPath
    const path = userPath ?? joinPath(tmpdir(), `hoshino-bench-${process.pid}-${Date.now()}.sqlite`)
...
async function buildPostgresStore(): Promise<BenchStoreFixture> {
    const { createPostgresStore } = await import('@revizahoshii/hoshino-store-postgres')
    const pg = await import('pg')
    const host = requireEnv('HOSHINO_TEST_PG_HOST')
    const port = parsePort('HOSHINO_TEST_PG_PORT')
    const user = process.env.HOSHINO_TEST_PG_USER ?? 'postgres'
    const password = process.env.HOSHINO_TEST_PG_PASSWORD ?? 'test'
    const database = process.env.HOSHINO_TEST_PG_DATABASE ?? 'hoshino_test'
    const tablePrefix = uniquePrefix('pg')

    const rawPool = new pg.Pool({ host, port, user, password, database, max: 16 })
    const trace = process.env.HOSHINO_BENCH_TRACE_QUERIES === '1'
...
async function buildMysqlStore(): Promise<BenchStoreFixture> {
    const { createMysqlStore } = await import('@revizahoshii/hoshino-store-mysql')
    const mysql = await import('mysql2/promise')
    const host = requireEnv('HOSHINO_TEST_MYSQL_HOST')
    const port = parsePort('HOSHINO_TEST_MYSQL_PORT')
    const user = process.env.HOSHINO_TEST_MYSQL_USER ?? 'root'
    const password = process.env.HOSHINO_TEST_MYSQL_PASSWORD ?? 'test'
    const database = process.env.HOSHINO_TEST_MYSQL_DATABASE ?? 'hoshino_test'
...
async function buildRedisStore(): Promise<BenchStoreFixture> {
    const { createRedisStore } = await import('@revizahoshii/hoshino-store-redis')
    const { Redis } = await import('ioredis')
    const host = requireEnv('HOSHINO_TEST_REDIS_HOST')
    const port = parsePort('HOSHINO_TEST_REDIS_PORT')
...
async function buildMongoStore(): Promise<BenchStoreFixture> {
    const { createMongoStore } = await import('@revizahoshii/hoshino-store-mongo')
    const { MongoClient } = await import('mongodb')
    const host = requireEnv('HOSHINO_TEST_MONGO_HOST')
    const port = parsePort('HOSHINO_TEST_MONGO_PORT')
    const uri = `mongodb://${host}:${port}/?directConnection=true`
    const database = `hoshino_bench_${Date.now().toString(36)}`
...
function requireEnv(name: string): string {
    ...
    if (!v) {
        throw new Error(
            `env ${name} is required for HOSHINO_BENCH_STORE=${process.env.HOSHINO_BENCH_STORE}; ` +
                `start the docker-compose stack or use HOSHINO_BENCH_STORE=memory`
        )
    }
    return v
}
