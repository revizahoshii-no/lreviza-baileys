 ...
 *
 * Tunables:
 *   HOSHINO_BENCH_CONNECT_ITERATIONS  (default 10)
 *
...
import { WaClient, type WaClientEventMap } from '@revizahoshii/hoshino'
    ...
    const client = new WaClient(
        {
            ...
            sessionId: `hoshino-connect-bench-rpc-${iteration}`,
...
async function runOneIteration(iteration: number): Promise<IterationResult> {
    ...
    const client = new WaClient(
        {
            store: storeFixture.store,
            sessionId: `hoshino-connect-bench-${iteration}`,
...
async function main(): Promise<void> {
    ...
    const iterations = readPositiveIntEnv('HOSHINO_BENCH_CONNECT_ITERATIONS', 10)
    const backendName = process.env.HOSHINO_BENCH_STORE ?? 'memory'
    const separate = argSet.has('--separate-process')

    console.log('@revizahoshii/hoshino connect-lifecycle bench')
    console.log('───────────────────────────────')
