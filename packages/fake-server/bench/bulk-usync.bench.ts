 ...
 *
 * Tunables:
 *   HOSHINO_BENCH_USYNC_SIZES   csv (default "50,200,500")
 *   HOSHINO_BENCH_USYNC_DEVICES (default 1) devices per member
 *
...
async function main(): Promise<void> {
    ...
    const sizes = readCsvEnv('HOSHINO_BENCH_USYNC_SIZES', ['50', '200', '500']).map((s) => {
        const n = Number.parseInt(s, 10)
        if (!Number.isFinite(n) || n <= 0) throw new Error(`invalid size "${s}"`)
        return n
    })
    const devicesPerMember = readPositiveIntEnv('HOSHINO_BENCH_USYNC_DEVICES', 1)
    const backendName = process.env.HOSHINO_BENCH_STORE ?? 'memory'
    const separate = argSet.has('--separate-process')

    console.log('@revizahoshii/hoshino bulk-usync bench')
    console.log('────────────────────────')
