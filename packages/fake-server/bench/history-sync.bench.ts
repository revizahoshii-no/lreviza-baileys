 ...
 *
 * Tunables:
 *   HOSHINO_BENCH_HISTORY_CHUNKS    (default 5)   chunks per scenario
 *   HOSHINO_BENCH_HISTORY_SCENARIOS (default "small,medium,large")
 *
...
import type { WaClientEventMap } from '@revizahoshii/hoshino'
...
async function main(): Promise<void> {
    ...
    const chunks = readPositiveIntEnv('HOSHINO_BENCH_HISTORY_CHUNKS', 5)
    const requested = readCsvEnv('HOSHINO_BENCH_HISTORY_SCENARIOS', ['small', 'medium', 'large'])
    const backendName = process.env.HOSHINO_BENCH_STORE ?? 'memory'
    const separate = argSet.has('--separate-process')

    console.log('@revizahoshii/hoshino history-sync bench')
    console.log('──────────────────────────')
