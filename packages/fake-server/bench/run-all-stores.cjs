 ...
 * `HOSHINO_TEST_*` env vars (set the same way `scripts/test-stores.cjs`
 * does).
 *
...
function ensureDocker(stores) {
    ...
    if (stores.includes('mysql')) {
        env.HOSHINO_TEST_MYSQL_HOST = 'localhost'
        env.HOSHINO_TEST_MYSQL_PORT = getComposePort('mysql', 3306)
        env.HOSHINO_TEST_MYSQL_USER = 'root'
        env.HOSHINO_TEST_MYSQL_PASSWORD = 'test'
        env.HOSHINO_TEST_MYSQL_DATABASE = 'hoshino_test'
    }
    if (stores.includes('postgres')) {
        env.HOSHINO_TEST_PG_HOST = 'localhost'
        env.HOSHINO_TEST_PG_PORT = getComposePort('postgres', 5432)
        env.HOSHINO_TEST_PG_USER = 'postgres'
        env.HOSHINO_TEST_PG_PASSWORD = 'test'
        env.HOSHINO_TEST_PG_DATABASE = 'hoshino_test'
    }
    if (stores.includes('redis')) {
        env.HOSHINO_TEST_REDIS_HOST = 'localhost'
        env.HOSHINO_TEST_REDIS_PORT = getComposePort('redis', 6379)
    }
    if (stores.includes('mongo')) {
        env.HOSHINO_TEST_MONGO_HOST = 'localhost'
        env.HOSHINO_TEST_MONGO_PORT = getComposePort('mongo', 27017)
    }
    return env
}
