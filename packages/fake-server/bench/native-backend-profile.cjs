...
const WORKLOAD = {
    HOSHINO_BENCH_STORE: 'sqlite',
    HOSHINO_BENCH_CONTACTS: '1000',
    HOSHINO_BENCH_CONTACT_DEVICES: '2',
    HOSHINO_BENCH_GROUPS: '4',
    HOSHINO_BENCH_GROUP_MEMBERS: '500',
    HOSHINO_BENCH_MESSAGES: '1000'
}
...
function classify(functionName, url) {
    ...
    if (
        ...
        low.includes('hoshino_native_wasm')
    )
        return 'wasm'
...
function runProfile(backend, outDir) {
    ...
    const env = { ...process.env, ...WORKLOAD, HOSHINO_NATIVE_BACKEND: backend }
    delete env.HOSHINO_XEDDSA_FORCE_JS
    delete env.HOSHINO_X25519_FORCE_JS
    const res = spawnSync(
        process.execPath,
        [
            '--expose-gc',
