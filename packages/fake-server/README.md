# @revizahoshii/hoshino-fake-server

In-process fake WhatsApp server that drives the real `@revizahoshii/hoshino` `WaClient` end-to-end - full Noise XX/IK handshake, QR pairing, Signal Protocol (X3DH + Double Ratchet), SenderKey for groups, media upload/download over self-signed HTTPS, app-state sync - all without touching WhatsApp servers.
...
```bash
npm i -D @revizahoshii/hoshino-fake-server @revizahoshii/hoshino
```

`@revizahoshii/hoshino` is a peer dependency - the fake server reuses the core's binary codec, crypto, and protos. Everything below works from any project; nothing requires this monorepo.

## Quick start

```ts
import { FakeWaServer } from '@revizahoshii/hoshino-fake-server'
import { createStore, WaClient } from '@revizahoshii/hoshino'
...
├── transport/               # Re-exports from @revizahoshii/hoshino (codec, crypto, protos)
└── __tests__/               # Cross-check test suite
    └── helpers/             # Shared test utilities (hoshino-client factory)
...
## Using with non-@revizahoshii/hoshino clients
...
npm --workspace=@revizahoshii/hoshino-fake-server run bench:messaging
...
HOSHINO_BENCH_SCENARIOS=send_group HOSHINO_BENCH_MESSAGES=2000 \
...
| ---------------------------- | ------- | ------------------------------------------------------------ |
| `HOSHINO_BENCH_CONTACTS`        | 1000    | Number of contacts                                           |
| `HOSHINO_BENCH_CONTACT_DEVICES` | 2       | Devices per contact                                          |
| `HOSHINO_BENCH_GROUPS`          | 4       | Number of groups                                             |
| `HOSHINO_BENCH_GROUP_MEMBERS`   | 500     | Members per group                                            |
| `HOSHINO_BENCH_MESSAGES`        | 1000    | Messages per scenario                                        |
| `HOSHINO_BENCH_SCENARIOS`       | all     | CSV of: `send_1to1`, `recv_1to1`, `send_group`, `recv_group` |
| `HOSHINO_BENCH_JSON`            | 0       | Set to `1` to print JSON results                             |
| `HOSHINO_BENCH_VERBOSE`         | 0       | Set to `1` to forward lib warns/errors                       |
...
```bash
# One-off via npx
npx @revizahoshii/hoshino-fake-server --port 5222 --peer 5511888@s.whatsapp.net --log
...
```bash
npm --workspace=@revizahoshii/hoshino-fake-server run cli -- --port 5222 --peer 5511888@s.whatsapp.net --log
```
...
```bash
npm --workspace=@revizahoshii/hoshino-fake-server test
