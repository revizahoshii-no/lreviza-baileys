...
const host = process.env.HOSHINO_TEST_REDIS_HOST
const port = process.env.HOSHINO_TEST_REDIS_PORT
let sessionSeed = 0
...
describe('store-redis integration', { timeout: 60_000 }, () => {
    ...
    it('auth: save, load, clear', async (t) => {
        if (!store) return t.skip('HOSHINO_TEST_REDIS_* not set')
...
describe('store-redis storeTtlMs', { timeout: 60_000 }, () => {
    ...
    it('data store expires written keys after the configured TTL window', async (t) => {
        if (!store || !redis) return t.skip('HOSHINO_TEST_REDIS_* not set')
