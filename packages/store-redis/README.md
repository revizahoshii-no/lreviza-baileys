# @revizahoshii/hoshino-store-redis

Redis-backed persistent store for [`@revizahoshii/hoshino`](https://www.npmjs.com/package/@revizahoshii/hoshino). Best fit when you already run Redis for caching and want stateless app instances that can share WhatsApp session state through a network database.
...
```bash
npm install @revizahoshii/hoshino-store-redis ioredis
```
...
import { createRedisStore } from '@revizahoshii/hoshino-store-redis'
