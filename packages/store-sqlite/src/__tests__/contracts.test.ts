...
import {
    ...
    WaThreadMemoryStore
} from '@revizahoshii/hoshino/store'
...
test('message store contract parity between memory and sqlite providers', async () => {
    ...
    const dir = await mkdtemp(join(tmpdir(), 'hoshino-message-contract-'))
    try {
...
test('thread/contact contract parity between memory and sqlite providers', async () => {
    ...
    const dir = await mkdtemp(join(tmpdir(), 'hoshino-mailbox-contract-'))
    try {
...
test('privacy token store contract parity between memory and sqlite providers', async () => {
    ...
    const dir = await mkdtemp(join(tmpdir(), 'hoshino-privacy-token-contract-'))
    try {
