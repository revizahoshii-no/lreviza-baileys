...
function resolveTestDriver(): WaSqliteDriver {
    const requested = process.env.HOSHINO_SQLITE_TEST_DRIVER
    if (!requested) {
        return 'auto'
    }
    ...
    if (!TEST_SQLITE_DRIVERS.includes(requested as WaSqliteDriver)) {
        throw new Error(
            `invalid HOSHINO_SQLITE_TEST_DRIVER "${requested}". Expected one of: ${TEST_SQLITE_DRIVERS.join(', ')}`
        )
    }
 ...
 * `HOSHINO_SQLITE_TEST_DRIVER` to pin a specific driver - CI uses it to run the
