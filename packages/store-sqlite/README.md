...
SQLite-backed persistent store for [`@revizahoshii/hoshino`](https://www.npmjs.com/package/@revizahoshii/hoshino). Suitable for single-process bots, dev sessions, and small-to-medium production deployments that don't need a network database.
...
# or on Bun
bun add @revizahoshii/hoshino-store-sqlite
# or with no native dependency at all (Node 22.13+)
npm install @revizahoshii/hoshino-store-sqlite
```
...
```ts
import { createSqliteStore, openSqliteConnection } from '@revizahoshii/hoshino-store-sqlite'
...
See the main [`@revizahoshii/hoshino`](../../README.md) docs for the full store contract and per-domain semantics.
