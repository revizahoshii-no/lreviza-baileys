# @revizahoshii/hoshino-voip

WhatsApp **VOIP / calling** plugin for [`@revizahoshii/hoshino`](https://github.com/revizahoshii-no/lreviza-baileys).
...
```bash
npm install @revizahoshii/hoshino @revizahoshii/hoshino-voip libmlow-wasm
```
...
| -------------- | -------------- | ----------------------------------------------- |
| `@revizahoshii/hoshino`      | yes            | `WaClient` and plugin host                      |
...
Importing from `@revizahoshii/hoshino-voip` applies `WaClient` type extensions (`client.voip` and `voip_*` events):

```ts
import { WaClient } from '@revizahoshii/hoshino'
import { voipPlugin, EndCallReason } from '@revizahoshii/hoshino-voip'
