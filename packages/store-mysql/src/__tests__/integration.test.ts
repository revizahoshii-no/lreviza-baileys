...
const host = process.env.HOSHINO_TEST_MYSQL_HOST
const port = process.env.HOSHINO_TEST_MYSQL_PORT
let sessionSeed = 0
...
describe('store-mysql integration', { timeout: 60_000 }, () => {
    ...
    before(async () => {
        ...
        pool = createMysqlPool({
            host,
            port: Number(port),
            user: process.env.HOSHINO_TEST_MYSQL_USER ?? 'root',
            password: process.env.HOSHINO_TEST_MYSQL_PASSWORD ?? 'test',
            database: process.env.HOSHINO_TEST_MYSQL_DATABASE ?? 'hoshino_test'
        })
    ...
    it('auth: save, load, clear', async (t) => {
        if (!store) return t.skip('HOSHINO_TEST_MYSQL_* not set')
    ...
    it('signal: setSessionsBatch chunked split runs in withTransaction', async (t) => {
        if (!pool) {
            t.skip('HOSHINO_TEST_MYSQL_* not set')
            return
        }
