# AGENTS.md – Development Guide for `hoshino`
...
```text
hoshino/
├── src/                # Core library source (@revizahoshii/hoshino)
...
├── packages/           # Optional packages (npm scope: @revizahoshii/hoshino)
...
- `examples/` runnable example scripts (can import `@revizahoshii/hoshino` and `@revizahoshii/hoshino-*` by name)
...
- `.github/workflows/release.yml` publishes to npm on a `v*` tag via trusted publishing (OIDC, no `NPM_TOKEN`): core (`@revizahoshii/hoshino`) through a guarded `npm publish`, add-ons (`@revizahoshii/hoshino-*`) through `changeset publish`
...
- **Core** (`@revizahoshii/hoshino`): published from the root. Build with `npm run build`.
- **Optional packages** (`@revizahoshii/hoshino-*`): live in `packages/<name>/`. Build all with `npx turbo run build`.
- Optional packages declare `@revizahoshii/hoshino` as a `peerDependency` – the end user installs core + the modules they want.
...
Optional packages (`@revizahoshii/hoshino-media-utils`, future store/transport addons)
...
`callerLogger.child({ scope: '@revizahoshii/hoshino-<name>' })`) on every call. This
...
SQLite provider rules (live in [`@revizahoshii/hoshino-store-sqlite`](packages/store-sqlite)):
...
- `createNoopLogger(level?)` – silent `Logger` for tests. Use instead of redefining a local `function createLogger(): Logger { return { level: 'trace', trace: () => undefined, child: () => ... } }`. Also re-exported from the public root (`'@revizahoshii/hoshino'`) so optional packages can import it the same way.
...
## 13. MCP Dev Server (`@revizahoshii/hoshino-mcp-server`)

Optional package that exposes `WaClient` sessions and the `@revizahoshii/hoshino` module namespace as MCP tools, so an LLM agent can drive end-to-end flows (connect, pair, send, query groups/newsletters, inspect events, etc.) without writing throwaway scripts. Source: `packages/mcp-server/`.
...
```bash
npm run build --workspace @revizahoshii/hoshino-mcp-server
claude mcp add hoshino --scope user -- node <abs-path>/packages/mcp-server/dist/bin.js
```

For tighter dev iteration, register the source via `tsx` (no build step needed):

```bash
claude mcp add hoshino --scope user -- node --import tsx <abs-path>/packages/mcp-server/src/bin.ts
```

### Tool surface

- **`call`** / **`inspect`** – walk dotted paths against `client` (the WaClient instance) or `lib` (the `@revizahoshii/hoshino` module namespace, including `proto.*` and helpers like `parsePhoneJid`). `call` invokes functions; `inspect` lists members with origin class.
...
```bash
npm run build --workspace @revizahoshii/hoshino-mcp-server
claude mcp add hoshino --scope user -- node <abs>/packages/mcp-server/dist/bin.js
```
...
| `MCP_FAKE_NOISE_PUBKEY_HEX` + `MCP_FAKE_NOISE_SERIAL` + `MCP_CHAT_SOCKET_URLS` | unset                         | point at `@revizahoshii/hoshino-fake-server` for tests             |
...
Changesets tracks the optional packages **only**, not the core. The root `package.json` (`@revizahoshii/hoshino`) is the workspace anchor, so `@manypkg`/Changesets exclude it from the releasable set – `npx changeset add` does not even list it.

- **Optional packages (`@revizahoshii/hoshino-*`)** – versioned with Changesets. Add a changeset in the PR that changes them: `npx changeset`, pick the packages and bump levels, write the summary. The file lands in `.changeset/`.
- **Core (`@revizahoshii/hoshino`)** – versioned by hand. Bump the root `version` directly with `npm version <patch|minor|major> --no-git-tag-version` (bumps the root only); Changesets never touches it. Its root `CHANGELOG.md` is likewise hand-maintained (see the release process below); `changeset version` only writes the add-on `packages/<name>/CHANGELOG.md` files, never the core's.
...
- [ ] `npm run lint`, `npm run format:check`, `npm run test`, and `npm run typecheck:all` pass locally (the `:all` variant catches package-level type errors that `npm run typecheck` alone misses – e.g. a public export removed from `src/<module>/index.ts` that a `packages/*` file imports via `@revizahoshii/hoshino/<module>`)
