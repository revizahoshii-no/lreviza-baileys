...
`@revizahoshii/hoshino` implements end-to-end-encrypted messaging primitives (Signal
...
- **Preferred:** GitHub Security Advisories – click
  [_Report a vulnerability_](https://github.com/revizahoshii-no/lreviza-baileys/security/advisories/new)
...
- Affected version (`@revizahoshii/hoshino` and any `@revizahoshii/hoshino-*` packages involved)
...
- Issues in optional packages (`@revizahoshii/hoshino-store-*`, `@revizahoshii/hoshino-media-utils`)
...
- We will not act on raw scanner output (Snyk, Dependabot, generic SAST
  reports) without a concrete proof-of-concept tied to `hoshino` code.
