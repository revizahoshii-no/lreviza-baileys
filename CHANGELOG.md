...
- Add an optional Rust-backed crypto backend, `@revizahoshii/hoshino-native`, for the messaging hot path: XEdDSA sign/verify (avoiding the WebCrypto Ed25519 round-trip that dominates SKDM sign in fanout) and X25519 ECDH scalar mult (avoiding the `createPrivateKey` / `createPublicKey` DER round-trip in `node:crypto.diffieHellman`). The binding is try-required at module load and falls back to the JS implementation when missing, so consumers without the prebuilt binary are unaffected. `HOSHINO_XEDDSA_FORCE_JS` and `HOSHINO_X25519_FORCE_JS` force the JS path.
...
- Expose `resolveWaDeviceIdentity`, `getWaBrowserDisplayName`, and `WA_VERSION` from the public barrels (plus `delay` from `@revizahoshii/hoshino/util`), so plugins and downstream code can build a client payload that agrees with the pairing device identity. Consumed by `@revizahoshii/hoshino-wam`.
...
- Add a `WaClient` plugin system via `defineWaClientPlugin`: a plugin either runs setup side effects or exposes an API at `client[exposeAs]`, with the accessor type and its events inferred from the plugin definition. Powers `@revizahoshii/hoshino-voip`.
...
- Build via the `prepare` lifecycle hook instead of `prepack`, so installing `@revizahoshii/hoshino` straight from a git URL ships a built `dist/` (npm runs `prepare`, not `prepack`, for git dependencies).
...
- Resolve the vendored spec bridges in the emitted `.d.ts` types and the ESM build, fixing `TS2307` on the published types and a runtime "Cannot find module" on `import '@revizahoshii/hoshino'` under ESM that shipped in 1.0.1.
...
- Resolve tsc-alias path mapping by pointing every `tsconfig.json` alias at its parent directory instead of an entry file, so emitted `.d.ts` files no longer contain unresolved `@module` / `@revizahoshii/hoshino/*` import specifiers.
...
- New packages: `@revizahoshii/hoshino-fake-server` (fake WhatsApp Web server for end-to-end testing,
  first publish) and `@revizahoshii/hoshino-media-utils` (ffmpeg/sharp processing and media message
  support).
